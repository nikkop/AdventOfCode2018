...
with open('input.txt') as input:
    ...
    minutes = max(minutes for minutes in guard_sleep.values())
    for guard in guard_sleep.items():
        if guard[1] == minutes:
            result = int(guard[0][1:]) * guard[1]

    """
    guards = set(re.search(r"#\w+", stamp).group() for stamp in stamps if re.search(r"#\w+", stamp))
    sleep_count = dict.fromkeys(guards, 0)
    """