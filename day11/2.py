    ...
    for x, row in enumerate(grid):
        for y, cell in enumerate(row):
            ...
            for i in range(1, grid_size+1):
                square_size = i
                if y <= grid_size - square_size + 1 and x <= grid_size - square_size + 1:
                    square = create_square(c, square_size)
                    power = sum(map(get_power_level, square))
                    if power > largest_power['power']:
                        largest_power['cell'] = c
                        largest_power['power'] = power
                        largest_power['size'] = square_size
